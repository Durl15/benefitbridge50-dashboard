...
export default function Admin( ) {
  ...
  const exportCSV=()=>{const headers=["Assessment ID","Date","State","County","Matches","Risk Flags"];const rows=filtered.map(a=>[a.assessment_id,new Date(a.created_at).toLocaleString(),a.state||"",a.county||"",a.match_count??0,a.risk_flag_count??0]);const csv=[headers,...rows].map(r=>r.map(v=>`"${v}"`).join(",")).join("\n");const blob=new Blob([csv],{type:"text/csv"});const url=URL.createObjectURL(blob);const el=document.createElement("a");el.href=url;el.download=`assessments_${new Date().toISOString().slice(0,10)}.csv`;el.click();URL.revokeObjectURL(url);};
  const toggleSort=(col)=>{if(sortBy===col)setSortDir(d=>d==="asc"?"desc":"asc");else{setSortBy(col);setSortDir("desc");}};
  const th={padding:"10px 14px",textAlign:"left",fontSize:"12px",fontWeight:"700",color:"#6B7280",textTransform:"uppercase",letterSpacing:"0.05em",borderBottom:"1px solid #E5E7EB",background:"#F9FAFB",cursor:"pointer",userSelect:"none",whiteSpace:"nowrap"};
  const td={padding:"12px 14px",fontSize:"14px",color:"#374151",borderBottom:"1px solid #F3F4F6"};
  return (
    <div style={{fontFamily:"system-ui, sans-serif",background:"#F9FAFB",minHeight:"100vh"}}>
      ...
      <div style={{maxWidth:"1200px",margin:"0 auto",padding:"32px 2rem"}}>
        <div style={{marginBottom:"28px"}}><h1 style={{fontSize:"26px",fontWeight:"800",color:"#0F2044",marginBottom:"4px"}}>Screener Submissions</h1><p style={{color:"#6B7280",fontSize:"14px"}}>All assessments submitted through BenefitBridge 50+</p></div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:"16px",marginBottom:"28px"}}>
          {[{label:"Total Screenings",value:assessments.length,color:"#0F2044"},{label:"Avg Benefit Matches",value:avgMatches,color:"#059669"},{label:"Total Risk Flags",value:totalRisk,color:"#DC2626"},{label:"States Covered",value:stateCount,color:"#7C3AED"}].map((s,i)=>(<div key={i} style={{background:"white",borderRadius:"12px",padding:"20px 24px",boxShadow:"0 1px 8px rgba(0,0,0,0.06)"}}><div style={{fontSize:"28px",fontWeight:"800",color:s.color}}>{s.value}</div><div style={{fontSize:"13px",color:"#6B7280",marginTop:"4px"}}>{s.label}</div></div>))}
        </div>
        <div style={{background:"white",borderRadius:"12px",padding:"16px 20px",boxShadow:"0 1px 8px rgba(0,0,0,0.06)",marginBottom:"16px",display:"flex",gap:"12px",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap"}}>
          <input style={{flex:1,minWidth:"200px",padding:"9px 14px",border:"1px solid #D1D5DB",borderRadius:"8px",fontSize:"14px",color:"#111827"}} placeholder="Search by state, county, or ID..." value={search} onChange={e=>setSearch(e.target.value)} />
          <div style={{display:"flex",gap:"8px",alignItems:"center"}}><span style={{fontSize:"13px",color:"#6B7280"}}>{filtered.length} results</span><button onClick={exportCSV} style={{background:"#0F2044",color:"white",padding:"9px 18px",borderRadius:"8px",border:"none",fontWeight:"600",fontSize:"13px",cursor:"pointer"}}>Export CSV</button></div>
        </div>
        <div style={{background:"white",borderRadius:"12px",boxShadow:"0 1px 8px rgba(0,0,0,0.06)",overflow:"hidden"}}>
          {loading?(<div style={{padding:"48px",textAlign:"center",color:"#6B7280"}}>Loading assessments...</div>):error?(<div style={{padding:"48px",textAlign:"center",color:"#DC2626"}}>{error}</div>):filtered.length===0?(<div style={{padding:"48px",textAlign:"center",color:"#6B7280"}}>No assessments found.</div>):(
        ...
        <p style={{marginTop:"16px",fontSize:"12px",color:"#9CA3AF",textAlign:"center"}}>Data from Railway backend. Assessments are anonymized.</p>
      </div>
    </div>
  );
}