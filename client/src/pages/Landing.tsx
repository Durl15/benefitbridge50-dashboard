...
export default function Landing() {
  ...
  const steps = [
    { num: "1", title: "Answer 10 questions", desc: "Tell us your age, income, location, and situation. Takes under 3 minutes." },
    { num: "2", title: "AI screens your eligibility", desc: "Our system checks hundreds of federal, state, and local programs instantly." },
    { num: "3", title: "Get your matched benefits", desc: "See exactly which programs you likely qualify for, with next steps to apply." },
  ];

  const testimonials = [
    { quote: "I had no idea I qualified for SNAP and LIHEAP. This saved me over $400 a month.", name: "Margaret T., 71 - Florida" },
    { quote: "Found out I was eligible for Medicare Extra Help. My prescription costs dropped to almost nothing.", name: "Robert K., 68 - Ohio" },
...
<span style={{ color: "#F59E0B" }}> entitled to in 3 minutes</span></h1>
      ...
      <section style={{ background: "#F9FAFB", padding: "48px 2rem" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "24px", textAlign: "center" }}>
          {stats.map((s, i) => (<div key={i} style={{ padding: "24px", background: "white", borderRadius: "12px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}><div style={{ fontSize: "32px", fontWeight: "800", color: "#0F2044" }}>{s.num}</div><div style={{ fontSize: "14px", color: "#6B7280", marginTop: "6px" }}>{s.label}</div></div>))}
        </div>
      </section>
      ...
      <section id="how-it-works" style={{ padding: "80px 2rem", textAlign: "center" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "32px" }}>
            {steps.map((step, i) => (<div key={i} style={{ textAlign: "center" }}><div style={{ width: "48px", height: "48px", borderRadius: "50%", background: "#0F2044", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "20px", fontWeight: "800", margin: "0 auto 16px" }}>{step.num}</div><div style={{ fontWeight: "700", fontSize: "16px", color: "#0F2044", marginBottom: "8px" }}>{step.title}</div><div style={{ fontSize: "14px", color: "#6B7280", lineHeight: 1.6 }}>{step.desc}</div></div>))}
          </div>
          <div style={{ marginTop: "48px" }}><Link href="/screen"><button style={{ background: "#0F2044", color: "white", padding: "14px 36px", borderRadius: "8px", border: "none", fontWeight: "700", fontSize: "16px", cursor: "pointer" }}>Start My Free Screening</button></Link></div>
        </div>
      </section>
      ...
      <section style={{ background: "#F9FAFB", padding: "80px 2rem" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "24px" }}>
            {testimonials.map((t, i) => (<div key={i} style={{ background: "white", borderRadius: "12px", padding: "24px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", borderTop: "3px solid #F59E0B" }}><p style={{ fontSize: "14px", color: "#374151", lineHeight: 1.7, marginBottom: "16px", fontStyle: "italic" }}>"{t.quote}"</p><div style={{ fontSize: "13px", fontWeight: "600", color: "#6B7280" }}>- {t.name}</div></div>))}
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "#0F2044", padding: "80px 2rem", textAlign: "center" }}>
        <div style={{ maxWidth: "520px", margin: "0 auto" }}>
          ...
          {submitted ? (
            <div style={{ background: "rgba(52,211,153,0.15)", border: "1px solid rgba(52,211,153,0.4)", borderRadius: "10px", padding: "20px", color: "#6EE7B7", fontSize: "16px", fontWeight: "600" }}>You are on the list! We will notify you of new benefits.</div>
          ) : (
            <form onSubmit={handleEmailSubmit} style={{ display: "flex", gap: "12px", maxWidth: "440px", margin: "0 auto" }}>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="your@email.com" style={{ flex: 1, padding: "12px 16px", borderRadius: "8px", border: "none", fontSize: "15px", color: "#111827", background: "white" }} />
              <button type="submit" disabled={submitting} style={{ background: "#F59E0B", color: "#0F2044", padding: "12px 24px", borderRadius: "8px", border: "none", fontWeight: "700", fontSize: "15px", cursor: "pointer", whiteSpace: "nowrap" }}>{submitting ? "..." : "Notify Me"}</button>
            </form>
          )}
        ...
        <div style={{ maxWidth: "800px", margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "48px", alignItems: "center" }}>
          <div>
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: "0 0 28px 0" }}>
              {["Embed on your website","Export results to CSV","Custom branding","Bulk screening for case managers"].map((f,i)=>(<li key={i} style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px", fontSize: "14px", color: "#374151" }}><span style={{ color: "#059669", fontWeight: "700" }}>+</span> {f}</li>))}
            </ul>
            <a href="mailto:contact@benefitbridge50.com" style={{ display: "inline-block", background: "#0F2044", color: "white", padding: "12px 28px", borderRadius: "8px", textDecoration: "none", fontWeight: "700", fontSize: "14px" }}>Contact Us About Partnerships</a>
          </div>
          <div style={{ background: "#F9FAFB", borderRadius: "16px", padding: "32px", textAlign: "center" }}>
            ...
            {[["Unlimited screenings",true],["White-label branding",true],["CSV export",true],["API access",false],["Dedicated support",false]].map(([f,inc],i)=>(<div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: i<4?"1px solid #E5E7EB":"none", fontSize: "14px" }}><span style={{ color: "#374151" }}>{f}</span><span style={{ color: inc?"#059669":"#D1D5DB", fontWeight: "600" }}>{inc?"+":"-"}</span></div>))}
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "#F9FAFB", padding: "60px 2rem", textAlign: "center" }}>
        ...
        <Link href="/screen"><button style={{ background: "#F59E0B", color: "#0F2044", padding: "14px 36px", borderRadius: "8px", border: "none", fontWeight: "800", fontSize: "16px", cursor: "pointer" }}>Screen for Benefits Now</button></Link>
      </section>

      <footer style={{ background: "#0F2044", color: "rgba(255,255,255,0.5)", padding: "24px 2rem", textAlign: "center", fontSize: "13px" }}>
        <div style={{ marginBottom: "8px" }}><span style={{ marginRight: "24px" }}>BenefitBridge 50+</span><a href="mailto:contact@benefitbridge50.com" style={{ color: "rgba(255,255,255,0.5)", textDecoration: "none", marginRight: "24px" }}>contact@benefitbridge50.com</a><Link href="/screen"><span style={{ color: "rgba(255,255,255,0.5)", cursor: "pointer" }}>Screener</span></Link></div>
