...
const useCases = [
  { org: "Senior Centers", icon: "SC", desc: "Screen walk-in clients in minutes instead of hours." },
  { org: "Hospitals & Health Systems", icon: "H+", desc: "Connect patients to SNAP, utility help, and housing support before they leave." },
  { org: "Area Agencies on Aging", icon: "AA", desc: "Equip case managers with a consistent screening tool." },
  { org: "Nonprofit Social Services", icon: "NP", desc: "Offer clients a free benefit check as part of intake." },
];
    ...
    <div style={{ fontFamily: "system-ui, sans-serif", color: "#1a1a2e", background: "white" }}>
      ...
      <section style={{ background: "linear-gradient(135deg, #0F2044 0%, #162B52 100%)", color: "white", padding: "72px 2rem", textAlign: "center" }}>
        <div style={{ maxWidth: "680px", margin: "0 auto" }}>
          ...
          <p style={{ fontSize: "17px", color: "rgba(255,255,255,0.8)", lineHeight: 1.6, marginBottom: "32px" }}>BenefitBridge 50+ gives your team an AI-powered screening tool in under 3 minutes per client.</p>
      ...
      <section style={{ padding: "60px 2rem", background: "white" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "24px", textAlign: "center" }}>
          {[{ num: "3 min", label: "Average screening time" },{ num: "6+", label: "Benefit programs matched" },{ num: "$400+", label: "Monthly value found per client" }].map((s, i) => (<div key={i} style={{ padding: "28px 16px", borderRadius: "12px", background: "#F9FAFB" }}><div style={{ fontSize: "36px", fontWeight: "800", color: "#0F2044" }}>{s.num}</div><div style={{ fontSize: "13px", color: "#6B7280", marginTop: "8px" }}>{s.label}</div></div>))}
        </div>
      </section>
      <section style={{ padding: "72px 2rem", background: "#F9FAFB" }}>
      ...
      <section style={{ padding: "72px 2rem", background: "white" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto" }}>
          ...
          <p style={{ color: "#6B7280", textAlign: "center", marginBottom: "40px" }}>Watch how BenefitBridge screens a client in under 3 minutes</p>
          <div style={{ position: "relative", paddingBottom: "56.25%", height: 0, overflow: "hidden", borderRadius: "12px", boxShadow: "0 8px 32px rgba(0,0,0,0.1)" }}>
            <iframe style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", border: "none" }} src="https://player.vimeo.com/video/1194406944" allow="autoplay; fullscreen; picture-in-picture" allowFullScreen />
          </div>
        </div>
      </section>
      <section id="contact" style={{ padding: "72px 2rem", background: "#F9FAFB" }}>
        <div style={{ maxWidth: "640px", margin: "0 auto" }}>
          <h2 style={{ fontSize: "28px", fontWeight: "800", color: "#0F2044", marginBottom: "8px", textAlign: "center" }}>Request a free demo</h2>
          <p style={{ color: "#6B7280", textAlign: "center", marginBottom: "40px" }}>We will reach out within 1 business day.</p>
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} style={{ background: "white", borderRadius: "16px", padding: "2rem", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "16px" }}>
                ...
                <div><label style={lbl}>Organization *</label><input style={inp} value={form.org} onChange={e=>sf("org",e.target.value)} required placeholder="Onondaga Senior Center" /></div>
                <div><label style={lbl}>Work Email *</label><input style={inp} type="email" value={form.email} onChange={e=>sf("email",e.target.value)} required placeholder="jane@org.org" /></div>
              ...
              <p style={{ marginTop: "12px", fontSize: "12px", color: "#9CA3AF", textAlign: "center" }}>No commitment required.</p>
            </form>
          )}
        </div>
      </section>
